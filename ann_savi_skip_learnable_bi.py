...
class ANNSAVISkipLearnableBI(nn.Module):
    ...
    def forward(self, x):
        band_3 = x[:,10]
        band_4 = x[:,3]
        bi = torch.sqrt((torch.square(band_3)+torch.square(band_3))/2).reshape(x.shape[0],1)
        l = torch.mean(self.linear1(bi))
        self.L = l
        band_8 = x[:,10]
        ...
        savi = savi.reshape(savi.shape[0],1)
        savi = torch.hstack((savi, band_8, band_4, l.repeat(x.shape[0],1)))
        x = self.linear2(savi)
        return x
